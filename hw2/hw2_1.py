...
class HashTable:
    ...
    def put(self, key, value):
        ...
        bucket_index = hash_num % self.bucket_size
        item = self.buckets[bucket_index]
        if item is None:
            self.buckets[bucket_index] = Item(key, value, None)
            self.item_count += 1
        else:
            prev_item = None
            while item is not None:
                ...
                prev_item = item
                item = item.next
            prev_item.next = Item(key, value, None)
            self.item_count += 1
    ...
    def rehash(self, new_bucket_size):
        ...
        for bucket in self.buckets:
            item = bucket
            while item is not None:
                hash_num = calculate_hash(item.key)
                bucket_index = hash_num % new_bucket_size
